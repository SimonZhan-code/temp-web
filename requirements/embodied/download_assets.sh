...
SUPPORT_LIST=("maniskill" "openpi" "safelibero")
ASSETS=()
...
download_safelibero_assets() {
	# Destination is inside the SAFETY_LIBERO package (not a runtime dir)
	local script_dir
	script_dir="$(readlink -f "$(dirname "${BASH_SOURCE[0]}")")"
	local dest_dir
	dest_dir="$(readlink -f "$script_dir/../../SAFETY_LIBERO/safelibero")"

	# Check if already downloaded
	if [ -d "$dest_dir/bddl_files/safelibero_spatial" ] && \
	   [ "$(ls -A "$dest_dir/bddl_files/safelibero_spatial" 2>/dev/null)" ]; then
		echo "[download_assets] SafeLIBERO assets already exist, skipping."
		return
	fi

	if [ ! -d "$dest_dir" ]; then
		echo "SAFETY_LIBERO package not found at $dest_dir. Run setup first." >&2
		exit 1
	fi

	echo "[download_assets] Cloning vlsa-aegis (shallow)..."
	local tmp
	tmp=$(mktemp -d)
	git clone --depth 1 https://github.com/THU-RCSCT/vlsa-aegis.git "$tmp"

	mkdir -p "$dest_dir/bddl_files" "$dest_dir/init_files"

	for suite in safelibero_spatial safelibero_object safelibero_goal safelibero_long; do
		SRC_BDDL="$tmp/safelibero/libero/libero/bddl_files/$suite"
		SRC_INIT="$tmp/safelibero/libero/libero/init_files/$suite"
		if [ -d "$SRC_BDDL" ]; then
			cp -r "$SRC_BDDL" "$dest_dir/bddl_files/"
		else
			echo "[WARNING] $SRC_BDDL not found in cloned repo." >&2
		fi
		if [ -d "$SRC_INIT" ]; then
			cp -r "$SRC_INIT" "$dest_dir/init_files/"
		else
			echo "[WARNING] $SRC_INIT not found in cloned repo." >&2
		fi
	done

	rm -rf "$tmp"
	echo "[download_assets] SafeLIBERO assets downloaded to $dest_dir"
}
