...
class LiberoEnv(gym.Env):
    def __init__(self, cfg, num_envs, seed_offset, total_num_processes, worker_info):
        ...
        self._compute_allowed_reset_state_ids()
    ...
    def get_env_fn_params(self, env_idx=None):
        ...
        for env_id in range(self.num_envs):
            ...
            task_bddl_file = self.task_suite.get_task_bddl_file_path(self.task_ids[env_id])
    ...
    def _get_random_reset_state_ids(self, num_reset_states):
        if self.specific_reset_id is not None:
            ...
            # Sample within the allowed (optionally scene-locked) reset-state set.
            pos = self._generator.integers(
                low=0, high=len(self.allowed_reset_state_ids), size=(num_reset_states,)
            )
            reset_state_ids = self.allowed_reset_state_ids[pos]
        return reset_state_ids

    def _compute_allowed_reset_state_ids(self):
        """Restrict the sampleable reset-state ids to a scene / task subset.
        ...
        allowed_task_ids = []
        for task_id in range(self.task_suite.get_num_tasks()):
            name = self.task_suite.get_task(task_id).name
            if scene_id is not None and not name.startswith(scene_id):
                continue
        ...
        for task_id in allowed_task_ids:
            ...
            end = int(self.cumsum_trial_id_bins[task_id])
            ranges.append(np.arange(start, end))
        self.allowed_reset_state_ids = np.concatenate(ranges)
        self.allowed_task_ids = allowed_task_ids

    @staticmethod
    def _count_goal_predicates(bddl_path):
        """Count predicate clauses inside a BDDL ``(:goal ...)`` block."""
        ...
        and_idx = block.lower().find("(and")
        if and_idx == -1:
            return 1  # a single bare predicate
        sub = block[and_idx + 4 :]
        d = 0
        count = 0
        for ch in sub:
    ...
    def get_reset_state_ids_all(self):
        reset_state_ids = self.allowed_reset_state_ids.copy()
