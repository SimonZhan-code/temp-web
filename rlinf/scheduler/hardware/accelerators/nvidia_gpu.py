...
class NvidiaGPUManager(AcceleratorManager):
    ...
    def get_accelerator_env_var(visible_accelerators: list[str]) -> dict[str, str]:
        ...
        # Propagate user-set NCCL transport/debug vars to every worker so collocated
        # IPC and cross-device NCCL weight sync honor them (e.g. NCCL_P2P_DISABLE=1 is
        # required on multi-GPU nodes without NVLink/direct-P2P, where P2P sends can
        # raise "CUDA error: illegal memory access").
        for _nccl_var in (
            "NCCL_P2P_DISABLE",
            "NCCL_SHM_DISABLE",
            "NCCL_IB_DISABLE",
            ...
            if _nccl_var in os.environ:
                env_vars[_nccl_var] = os.environ[_nccl_var]
