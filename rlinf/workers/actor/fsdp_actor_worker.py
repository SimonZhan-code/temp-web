...
class EmbodiedFSDPActor(FSDPModelManager, Worker):
    ...
    def init_worker(self) -> None:
        ...
        if self.enable_offload:
            ...
            self.offload_optimizer()
        self._setup_rollout_weight_dst_ranks()
        self._init_vmpo_optimizer()

    def _init_vmpo_optimizer(self) -> None:
        """Initialize V-MPO temperature dual + safety multiplier if enabled.

        Single-critic on-policy method: a one-variable temperature dual (η*, from the
        top-half advantage filter) plus a scalar safety multiplier λ in the reward
        (fixed β or adaptive dual-ascent). No-op when ``algorithm.vmpo`` is absent.
        """
        self.vmpo_temp = None
        self.safety_lambda = None
        vmpo_cfg = self.cfg.algorithm.get("vmpo", None)
        if vmpo_cfg is None:
            return

        from rlinf.algorithms.dual import (
            SafetyLagrangeMultiplier,
            VMPOTemperatureOptimizer,
        )

        device = f"cuda:{int(os.environ['LOCAL_RANK'])}"
        self.vmpo_temp = VMPOTemperatureOptimizer(
            eta_init=vmpo_cfg.get("eta_init", 1.0),
            eta_min=vmpo_cfg.get("eta_min", 0.01),
            epsilon_eta=vmpo_cfg.get("epsilon_eta", 0.1),
            lr_eta=vmpo_cfg.get("lr_eta", 0.01),
        ).to(device)
        self.safety_lambda = SafetyLagrangeMultiplier(
            mode=vmpo_cfg.get("safety_mode", "fixed"),
            beta=vmpo_cfg.get("beta", 0.0),
            lambda_init=vmpo_cfg.get("lambda_init", 0.0),
            epsilon_1=vmpo_cfg.get("epsilon_1", 0.0),
            alpha_lambda=vmpo_cfg.get("alpha_lambda", 0.05),
            lambda_max=vmpo_cfg.get("lambda_max", 100.0),
        ).to(device)
        self.log_info(
            f"V-MPO optimizer initialized: η={vmpo_cfg.get('eta_init', 1.0)}, "
            f"ε_η={vmpo_cfg.get('epsilon_eta', 0.1)}, "
            f"safety_mode={vmpo_cfg.get('safety_mode', 'fixed')}, "
            f"β={vmpo_cfg.get('beta', 0.0)}"
        )
    ...
    @staticmethod
    def _hazard_indicator(cost_rewards: torch.Tensor) -> torch.Tensor:
        """Convert the signed safety margin {-1, +1} to the {0, 1} hazard indicator.

        The env emits ``cost_rewards`` as a signed HJ margin h(s) (+1 = avoid-set
        violated, -1 = safe). The scalarized reward and J_c need the indicator
        c_hazard = 1[L(s) ∈ A⁻]; a task with no avoid set keeps h = -1 ⇒ c_hazard = 0
        (reward-only / reach-only behavior).
        """
        return (cost_rewards > 0).to(cost_rewards.dtype)

    def _discounted_cost_return(
        self,
        c_hazard: torch.Tensor,
        dones: torch.Tensor,
        gamma: float,
        loss_mask: torch.Tensor | None,
    ) -> float:
        """Measured discounted cost return J_c of the current best-of-N policy.

        Computes Σ_t γ^t c_hazard along each trajectory (resetting at episode
        boundaries) and averages the per-step discounted-cost over valid steps.
        Mirrors the backward per-episode accumulation used in ``calculate_scores``.

        Args:
            c_hazard: {0,1} hazard indicator, shape [n_chunk, bsz, chunk] or [T, bsz].
            dones: Done flags, shape [n_chunk+1, bsz, chunk] or [T+1, bsz].
            gamma: Discount factor.
            loss_mask: Optional validity mask matching ``c_hazard``.

        Returns:
            Scalar J_c (float).
        """
        c = c_hazard.reshape(c_hazard.shape[0], -1)  # [T, B*]
        d = dones.reshape(dones.shape[0], -1)  # [T+1, B*]
        T = c.shape[0]
        running = torch.zeros_like(c[0])
        returns = torch.zeros_like(c)
        for step in reversed(range(T)):
            running = c[step] + gamma * running * (~d[step + 1].bool())
            returns[step] = running

        if loss_mask is not None:
            m = loss_mask.reshape(loss_mask.shape[0], -1).bool()
            denom = m.sum().clamp(min=1)
            return (returns * m).sum().item() / denom.item()
        return returns.mean().item()

    def _update_vmpo_temperature_and_lambda(
        self, advantages: torch.Tensor
    ) -> dict[str, float]:
        """Update the V-MPO temperature η* and safety multiplier λ; sync to model.

        Sets ``model.dual_eta = η*`` so that the best-of-N scorer (V_{r̃}/η*) uses the
        fresh temperature. The safety multiplier λ lives in the reward, not the scorer.
        """
        if self.vmpo_temp is None:
            return {}

        loss_mask = self.rollout_batch.get("loss_mask", None)
        metrics = self.vmpo_temp.step(advantages=advantages, mask=loss_mask)

        # Measure J_c from the {0,1} hazard indicator and update λ.
        cost_rewards = self.rollout_batch.get("cost_rewards", None)
        if cost_rewards is not None and self.safety_lambda is not None:
            c_hazard = self._hazard_indicator(cost_rewards)
            jc = self._discounted_cost_return(
                c_hazard,
                self.rollout_batch["dones"],
                self.cfg.algorithm.get("gamma", 0.99),
                loss_mask,
            )
            metrics.update(self.safety_lambda.step(measured_cost_return=jc))

        # Sync η* to model buffer so it propagates via state_dict to rollout workers.
        model = self.model
        if hasattr(model, "module"):
            model = model.module
        if hasattr(model, "dual_eta"):
            model.dual_eta.fill_(self.vmpo_temp.eta.item())
    ...
    def compute_advantages_and_returns(self) -> dict[str, torch.Tensor]:
        ...
        Compute the advantages and returns.
        """
        if self.cfg.algorithm.adv_type == "vmpo":
            return self._compute_vmpo_advantages_and_returns()

        value_rewards = self.rollout_batch.get("reach_rewards", self.rollout_batch["rewards"])
        kwargs = {
    ...
    @staticmethod
    def _flatten_step_signal(
        signal: torch.Tensor, reward_type: str
    ) -> torch.Tensor:
        """Flatten the per-step subgoal-boundary signal [n_chunk, bsz, chunk] → [T, bsz].

        Mirrors the chunk-level reduction + transpose/reshape that
        ``preprocess_embodied_advantages_inputs`` applies to rewards/dones, so the
        boundary aligns with the flattened reward/value/done tensors. Uses **max** over
        the chunk (same as dones): the segment terminates if the active subgoal is
        satisfied at *any* action within the chunk.
        """
        if reward_type == "chunk_level":
            signal = signal.max(dim=-1, keepdim=True)[0]
        n_chunk, bsz, csz = signal.shape
        return signal.transpose(1, 2).reshape(n_chunk * csz, bsz)

    @staticmethod
    def _derive_segment_group_ids(
        segment_dones: torch.Tensor, bsz: int
    ) -> torch.Tensor:
        """Per-(column, segment) group ids [T, bsz] from segment_dones [T+1, bsz].

        Each sub-trajectory between two subgoal boundaries becomes its own per-subgoal
        normalization group (one occurrence of a subgoal == one group).
        """
        inner = segment_dones[1:].long()  # [T, bsz]; inner[t]=1 if step t ends a segment
        seg_idx = torch.cumsum(inner, dim=0) - inner  # ends strictly before t (exclusive)
        col = torch.arange(bsz, device=segment_dones.device).unsqueeze(0)
        return seg_idx * bsz + col  # unique per (column, segment)

    def _compute_vmpo_advantages_and_returns(self) -> dict[str, torch.Tensor]:
        """Advantages/returns for the single-critic on-policy V-MPO method.

        Scalarizes the reach-avoid reward r̃ = r_reach − λ·c_hazard, then runs
        **subgoal-segmented** on-policy GAE (κ→1): each subgoal-satisfaction step is a
        soft terminal so the σ-only-conditioned value never bootstraps across a subgoal
        switch. Advantages are normalized **per subgoal** before the V-MPO temperature
        top-half filter. No HJ max-Bellman head, no off-policy cost critic.
        """
        from rlinf.algorithms.advantages import (
            compute_segmented_gae,
            per_subgoal_normalize,
        )
        from rlinf.algorithms.utils import (
        ...
        # --- Rollout-batch signals consumed here (each [n_chunk, bsz, chunk]) ---
        # reach_rewards:     r_reach, the per-step reach progress / subgoal-satisfaction
        #                    reward; falls back to the env task reward for non-LTL envs.
        # cost_rewards:      signed safety margin h(s) ∈ {-1, +1} (+1 = avoid set entered);
        #                    converted to the {0,1} hazard indicator c_hazard below.
        # dones:             episode termination (task complete or max steps).
        # subgoal_advanced:  1 at steps where the active subgoal was satisfied — the V-MPO
        #                    segment boundary (a soft terminal for the value bootstrap).
        #                    Optional: absent ⇒ no segmentation (episode-level GAE only).
        reach_rewards = self.rollout_batch.get(
            "reach_rewards", self.rollout_batch["rewards"]
        )
        cost_rewards = self.rollout_batch.get("cost_rewards", None)

        # Scalarize: r̃ = r_reach − λ·c_hazard. No avoid set ⇒ c_hazard = 0 ⇒ r̃ = r_reach.
        lam = self.safety_lambda.lam.item() if self.safety_lambda is not None else 0.0
        if cost_rewards is not None and lam != 0.0:
            c_hazard = self._hazard_indicator(cost_rewards)
            scalarized_rewards = reach_rewards - lam * c_hazard
        else:
            scalarized_rewards = reach_rewards

        reward_type = self.cfg.algorithm.reward_type
        loss_mask = self.rollout_batch.get("loss_mask", None)

        # Flatten rewards/values/dones/loss_mask to [T, bsz] / [T+1, bsz].
        ...
        num_chunk, chunk_size = kwargs["num_chunk"], kwargs["chunk_size"]
        bsz = kwargs["batch_size"]
        flat_rewards = kwargs["rewards"]  # [T, bsz]
        flat_values = kwargs["values"]  # [T+1, bsz]
        flat_dones = kwargs["dones"]  # [T+1, bsz]
        flat_loss_mask = kwargs.get("loss_mask", None)  # [T, bsz] or None

        # Subgoal-segmentation boundary (1 where the active subgoal was satisfied),
        # flattened + chunk-collapsed to align with the GAE tensors. None ⇒ no boundary.
        subgoal_advanced = self.rollout_batch.get("subgoal_advanced", None)
        segment_boundary = None
        if subgoal_advanced is not None:
            segment_boundary = self._flatten_step_signal(subgoal_advanced, reward_type)

        advantages, returns, segment_dones = compute_segmented_gae(
        ...
        # contribute comparably and the filter is not dominated by high-value subgoals.
        per_subgoal = self.cfg.algorithm.get("vmpo", {}).get(
            "per_subgoal_adv_norm", True
        )
        if per_subgoal:
            group_ids = self._derive_segment_group_ids(segment_dones, bsz)
            advantages = per_subgoal_normalize(advantages, group_ids, flat_loss_mask)
        else:
            advantages = safe_normalize(advantages, loss_mask=flat_loss_mask)

        # Unflatten back to [n_chunk, bsz, chunk].
        advantages_and_returns = postprocess_embodied_advantages_outputs(
            advantages=advantages,
            returns=returns,
            num_chunk=num_chunk,
        ...
        self.rollout_batch.update(advantages_and_returns)
        # NOTE: do not write back loss_mask/loss_mask_sum — preprocess flattened them to
        # [T, bsz], but the train loop expects the original [n_chunk, bsz, chunk] mask
        # already present in rollout_batch (we never modify its content here).

        # Update temperature η* (top-half filter) and safety multiplier λ; sync buffers.
        vmpo_metrics = self._update_vmpo_temperature_and_lambda(
            advantages_and_returns["advantages"]
        )

        rollout_metrics = compute_rollout_metrics(self.rollout_batch)
        if vmpo_metrics:
            rollout_metrics.update(vmpo_metrics)
        return rollout_metrics
        ...
        for _ in range(update_epoch):
            ...
            for train_global_batch in rollout_dataloader_iter:
                ...
                for idx, data in enumerate(train_micro_batch):
                    ...
                    compute_values = self.cfg.algorithm.adv_type in ("gae", "vmpo")
