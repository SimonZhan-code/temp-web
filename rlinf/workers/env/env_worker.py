...
class EnvWorker(Worker):
    def __init__(self, cfg: DictConfig):
        ...
        self._ltl_debug_print_count = 0
        self._ltl_debug_print_limit = 10

    def _extract_atomic_props(self, infos: dict) -> dict | None:
        if not infos:
            return None
        keys = ("ltl_label", "ltl_label_array", "ltl_goal_desc")
        atomic_props = {key: infos[key] for key in keys if key in infos}
        return atomic_props or None

    def _log_goal_props(self, infos: dict, stage_id: int) -> None:
        # if self._ltl_debug_print_count >= self._ltl_debug_print_limit:
        #     return
        if "ltl_label" not in infos:
            return
        labels = infos["ltl_label"]
        if not isinstance(labels, list) or not labels:
            return
        label_dict = labels[0]
        if not isinstance(label_dict, dict):
            return
        goal_desc_list = infos.get("ltl_goal_desc", [])
        goal_desc = (
            goal_desc_list[0]
            if isinstance(goal_desc_list, list) and goal_desc_list
            else {}
        )
        if isinstance(goal_desc, dict) and goal_desc:
            goal_names = set(goal_desc.keys())
        else:
            goal_names = {name for name in label_dict if name.startswith("goal_")}
        goal_props = {name: value for name, value in label_dict.items() if name in goal_names}
        if not goal_props:
            return
        print(
            f"[EnvWorker rank={self._rank} stage={stage_id}] goal props: "
            f"{sorted(goal_props.items())}"
        )
        self._ltl_debug_print_count += 1
    ...
    def env_interact_step(
        ...
        env_output = EnvOutput(
            ...
            atomic_props=self._extract_atomic_props(infos),
        )
