...
class OpenPi0ForRLActionPrediction(BasePolicy, PI0Pytorch):
    ...
    def __init__(
        ...
        if self.config.value_after_vlm:
            ...
            proj_width = 1024
        # value head
        # Value head connected after VLM output for pi05
        ...
        # V-MPO temperature η* (synced from the actor to rollout via state_dict);
        # the best-of-N score is V_{r̃}/η*.
        self.register_buffer(
            "dual_eta", torch.tensor(self.config.best_of_n_eta)
        )
    ...
    def obs_processor(self, env_obs):
        # base observation — append reach-avoid subgoal text to prompt if available
        prompts = env_obs["task_descriptions"]
        reach_avoid_texts = env_obs.get("reach_avoid_texts")
        if reach_avoid_texts is not None:
            prompts = [
                f"{p} | {ra}" if ra else p
                for p, ra in zip(prompts, reach_avoid_texts)
            ]
        processed_obs = {
    ...
    def sample_mean_var_val(
        ...
        """
        Sample the mean, variance and value of the action at a given timestep with low level flow-matching action.
    ...
    @torch.no_grad()
    def _sample_actions_best_of_n(
        self,
        observation: _model.Observation,
        mode="eval",
    ) -> dict:
        """Best-of-N sampling: generate N action candidates, score by Lagrangian, select best.

        Reuses the prefix KV cache across all N candidates for efficiency.
        Only used at eval time when config.best_of_n > 1.
        """
        ...
        # Compute prefix (VL backbone) once
        prefix_embs, prefix_pad_masks, prefix_att_masks = self.embed_prefix(
            images, img_masks, lang_tokens, lang_masks
        )
        prefix_att_2d_masks = make_att_2d_masks(prefix_pad_masks, prefix_att_masks)
        ...
        # VLM-based values are computed from prefix (observation-only), same for all N
        if self.use_vlm_value:
            vlm_values = self.get_value_from_vlm(prefix_output)  # [bsize]

        # Expand prefix state for N candidates: [bsize] -> [bsize * N]
        # Clone KV cache since batch_repeat_interleave mutates in-place
        from transformers.cache_utils import DynamicCache
        kv_expanded = DynamicCache()
        for layer_idx in range(len(past_key_values)):
            kv_expanded.update(
                past_key_values.key_cache[layer_idx].repeat_interleave(N, dim=0),
                past_key_values.value_cache[layer_idx].repeat_interleave(N, dim=0),
                layer_idx,
            )
        state_expanded = state.repeat_interleave(N, dim=0)
        prefix_pad_masks_expanded = prefix_pad_masks.repeat_interleave(N, dim=0)

        # Sample N different noises
        actions_shape = (bsize * N, self.config.action_horizon, self.config.action_dim)
        noise = self.sample_noise(actions_shape, device)
        x_t = noise

        # Run denoising loop on expanded batch (eval mode = ODE, no noise injection)
        all_values = []
        for idx in range(num_steps):
            x_t_mean, x_t_std, value_t = self.sample_mean_var_val(
                x_t, idx, state_expanded, prefix_pad_masks_expanded,
                kv_expanded, "eval", num_steps, compute_values=True,
            )
            x_t = x_t_mean  # ODE step (std=0 in eval mode)
            all_values.append(value_t)

        x_0 = x_t  # [bsize * N, action_horizon, action_dim]

        # Aggregate values across denoise steps
        if self.use_vlm_value:
            # VLM values don't depend on denoising trajectory, repeat for N
            values = vlm_values.repeat_interleave(N)  # [bsize * N]
        else:
            values = torch.stack(all_values, dim=1).mean(dim=1)  # [bsize * N]
        ...
        scores = values / eta  # [bsize, N]

        # Select best candidate per batch element
        best_idx = scores.argmax(dim=1)  # [bsize]
        best_actions = x_0[torch.arange(bsize), best_idx]  # [bsize, action_horizon, action_dim]
        best_values = values[torch.arange(bsize), best_idx]  # [bsize]

        # Build result compatible with sample_actions() output
        # For BoN eval we don't need training-specific fields (chains, logprobs, denoise_inds)
        # but we provide dummy values for pipeline compatibility
        dummy_chains = best_actions.unsqueeze(1)  # [bsize, 1, action_horizon, action_dim]
        dummy_logprobs = torch.zeros(bsize, self.config.action_chunk, self.config.action_env_dim, device=device)
        ...
        result = {
            ...
            "prev_values": best_values[:, None],  # [bsize, 1]
            "denoise_inds": dummy_denoise_inds,
            "best_of_n_scores": scores,  # [bsize, N] for diagnostics
            "best_of_n_idx": best_idx,  # [bsize] for diagnostics
        }
        return result
    ...
    def get_value_from_vlm(self, prefix_output):
        '''
        Take VLM output and compute value estimation with defined value head.
        '''
        # prefix_output:
