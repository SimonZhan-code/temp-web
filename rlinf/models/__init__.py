...
def get_model(cfg: DictConfig, override_config_kwargs=None):
    ...
    if model_type == SupportedModel.OPENVLA:
        ...
        # train expert only
        # Seems like we only freeze VLM backbone for openpi series models when training with RL finetuning.
    ...
    elif model_type == SupportedModel.RANDOM_POLICY:
        from .embodiment.random_policy import RandomPolicy

        gripper_indices = cfg.get("gripper_indices", None)
        if gripper_indices is not None:
            gripper_indices = list(OmegaConf.to_container(gripper_indices, resolve=True))
        model = RandomPolicy(
            action_dim=cfg.action_dim,
            num_action_chunks=cfg.num_action_chunks,
            action_low=cfg.get("action_low", -1.0),
            action_high=cfg.get("action_high", 1.0),
            gripper_mode=cfg.get("gripper_mode", "none"),
