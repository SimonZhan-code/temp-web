...
@register_policy_loss("vmpo")
def compute_vmpo_loss(**kwargs) -> tuple[torch.Tensor, dict]:
    """Single-critic on-policy V-MPO loss.

    By default this is **critic-only**: the single value head V_{r̃} regresses to the
    scalarized λ-returns. No policy weights are updated — the frozen backbone +
    best-of-N selection (scored by V_{r̃}/η* at rollout time) is the implicit policy
    improvement. Only V_{r̃}, the temperature η*, and (optionally) λ are learned.

    An optional parametric V-MPO weighted actor loss can be added later (gated by
    ``vmpo_actor_loss``) to enable backbone LoRA fine-tuning: weight the top-half
    samples by softmax(Ã/η*) and add a clipped policy-gradient term here.

    Returns:
        Tuple[torch.Tensor, Dict]: (loss, metrics_dict)
    """
    metrics_data = {}
    critic_loss, critic_metrics_data = compute_ppo_critic_loss(**kwargs)
    loss = critic_loss
    ...
    # OPTIONAL (default OFF): parametric V-MPO exp-weighted actor loss for fine-tuning.
    if kwargs.get("vmpo_actor_loss", False):
        actor_loss, actor_metrics_data = compute_ppo_actor_loss(**kwargs)
