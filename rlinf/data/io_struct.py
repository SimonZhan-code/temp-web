...
class ChunkStepResult:
    ...
    reach_rewards: torch.Tensor = None  # [B, 1]  (LTL reach signal for V_r)
    cost_rewards: torch.Tensor = None  # [B, 1]  (LTL cost signal for V_c)
    subgoal_advanced: torch.Tensor = None  # [B, 1]  (1 if active subgoal satisfied this step → V-MPO segment boundary)
    ...
    def __post_init__(self):
        ...
        if self.reach_rewards is not None:
            self.reach_rewards = self.reach_rewards.cpu().contiguous()
        if self.cost_rewards is not None:
            self.cost_rewards = self.cost_rewards.cpu().contiguous()
        if self.subgoal_advanced is not None:
            self.subgoal_advanced = self.subgoal_advanced.cpu().contiguous()
...
class EmbodiedRolloutResult:
    ...
    reach_rewards: list[torch.Tensor] = field(
        default_factory=list
    )  # lens of results is rollout_epoch * n_chunk_steps (LTL reach signal for V_r)
    cost_rewards: list[torch.Tensor] = field(
        default_factory=list
    )  # lens of results is rollout_epoch * n_chunk_steps (LTL cost signal for V_c)
    subgoal_advanced: list[torch.Tensor] = field(
        default_factory=list
    )  # lens of results is rollout_epoch * n_chunk_steps (V-MPO segment boundary)
    ...
    def append_result(self, result: ChunkStepResult):
        ...
        if result.reach_rewards is not None:
            self.reach_rewards.append(result.reach_rewards)
        if result.cost_rewards is not None:
            self.cost_rewards.append(result.cost_rewards)
        if result.subgoal_advanced is not None:
            self.subgoal_advanced.append(result.subgoal_advanced)
    ...
    def to_dict(self):
        ...
        # Emit reach/cost rewards only when the env actually produced them. A None value
        # crashes cat_list_of_dict_tensor on the actor receive path (it rejects non-tensor
        # values), so when the buffer is empty we omit the key entirely and let the actor's
        # .get("reach_rewards", rewards) fall back to the task reward (same pattern as
        # subgoal_advanced below).
        if len(self.reach_rewards) > 0:
            rollout_result_dict["reach_rewards"] = (
                torch.stack(self.reach_rewards, dim=0).cpu().contiguous()
            )
        if len(self.cost_rewards) > 0:
            rollout_result_dict["cost_rewards"] = (
                torch.stack(self.cost_rewards, dim=0).cpu().contiguous()
            )
        # Only emit the V-MPO subgoal signals when the env actually produced them.
        # Unlike reach/cost rewards (always present for LTL envs), these may be absent;
        # a None value would crash cat_list_of_dict_tensor on the actor receive path, so
        # we omit the key entirely and let the actor's .get(...) default to None
        # (segmentation disabled).
        if len(self.subgoal_advanced) > 0:
            rollout_result_dict["subgoal_advanced"] = (
                torch.stack(self.subgoal_advanced, dim=0).cpu().contiguous()
            )
