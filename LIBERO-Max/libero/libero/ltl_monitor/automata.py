"""LDBA primitives for LIBERO-Max LTL monitoring.

Ported from Neuralsym-VLA/ltl_benchmark/automata/ldba.py. Provides a full
limit-deterministic Buchi automaton with epsilon transitions, SAT-based
transition guards, sink completion, SCC computation, and pruning.

This replaces the earlier hand-crafted ``build_eventually_all_ldba`` /
``build_sequence_ldba`` template builders. The HOA→LDBA path is in
``hoa_parser.py`` and the entry point that wires Rabinizer and the cached-HOA
fallback is in ``builder.build_ldba``.
"""
...
from .logic import Assignment, FrozenAssignment


class LDBA:
    def __init__(self, propositions, formula: Optional[str] = None):
        self.formula = formula
        self.propositions: tuple[str, ...] = tuple(sorted(set(propositions)))
        self.num_states = 0
        ...
        self.state_to_scc: dict[int, "SCC"] = {}

    @property
    def states(self) -> list[int]:
        return list(range(self.num_states))

    def add_state(self, state: int, initial: bool = False) -> None:
        if state < 0:
            raise ValueError("State must be a positive integer.")
        if initial:
            if self.initial_state is not None and self.initial_state != state:
    ...
    def add_transition(
        ...
        for t in self.state_to_transitions[source]:
            if t == transition:
                raise ValueError(
                    "There can only be a single transition between two states. "
                    "Consider merging labels."
                )
    ...
    def get_next_states(
        ...
        take_epsilon: bool = False,
    ) -> List[Tuple[int, bool]]:
        """Given current state and the set of true propositions, return all
        ``(next_state, accepting)`` reachable by a single transition.

        If ``take_epsilon`` is True, returns the (single) epsilon successor.
        """
        if take_epsilon:
            eps_transitions = [
            ...
            if len(eps_transitions) > 1:
                raise ValueError(
                    "More than one epsilon transition from a state is currently "
                    "not supported."
                )
        ...
        assignment = Assignment(
            {p: (p in propositions) for p in self.propositions}
        ).to_frozen()
        outcomes = [
            (transition.target, transition.accepting)
    ...
    def is_state_violating(self, state: int) -> bool:
        scc = self.state_to_scc[state]
        return scc.bottom and not scc.accepting
    ...
    def complete_sink_state(self) -> None:
        """Add a sink state covering any assignment not handled by an existing
        outgoing transition. Idempotent."""
        if self.complete:
            return
        ...
        if self.possible_assignments:
            ...
            all_assignments = set(
                [
                    a.to_frozen()
                    for a in Assignment.all_possible_assignments(
                        tuple(self.propositions)
                    )
                ]
            )
        for state in range(self.num_states):
            ...
            if len(covered_assignments) != len(all_assignments):
                ...
                sink_label = self._valid_assignments_to_label(sink_assignments)
    ...
    def _valid_assignments_to_label(
        self, valid_assignments: set[FrozenAssignment]
    ) -> str:
        assert len(valid_assignments) > 0
        return " | ".join("(" + a.to_label() + ")" for a in valid_assignments)

    def prune(self, possible_assignments: list[Assignment]) -> None:
        self.possible_assignments = possible_assignments
        to_remove: set[LDBATransition] = set()
        for transitions in self.state_to_transitions.values():
            for t in transitions:
                ...
                t.valid_assignments = {
                    ...
                    if a.satisfies(t.label)
                }
                t.feasible_assignments = t.valid_assignments.copy()
                if t.valid_assignments:
                    t.label = self._valid_assignments_to_label(t.valid_assignments)
                else:
                    to_remove.add(t)
    ...
    def compute_sccs(self) -> None:
        ...
        def tarjan(s: int):
            nonlocal num, nums, visited, stack, active
            visited.add(s)
            active.add(s)
            num += 1
            nums[s] = num
            ...
            if stack[-1][0] == s:
                ...
                accepting = any(
                    t.accepting and t.target in states for t in transitions
                )
    ...
    def mark_unfeasible(
        self, states: list[int], assignment: FrozenAssignment
    ) -> None:
        for state in states:
            for t in self.state_to_transitions[state]:
                try:
                    t.feasible_assignments.remove(assignment)
                except KeyError:
                    continue
...
class LDBATransition:
    ...
    valid_assignments: set[FrozenAssignment] = field(init=False)
    feasible_assignments: set[FrozenAssignment] = field(init=False)

    def __post_init__(self):
        if self.label is None:
            self.valid_assignments = set()
            self.feasible_assignments = set()
        else:
            ...
            self.feasible_assignments = self.valid_assignments.copy()

    def is_epsilon(self) -> bool:
        return self.label is None

    def is_feasible(self) -> bool:
        return self.label is None or bool(self.feasible_assignments)
    ...
    def positive_label(self) -> str:
        if self.is_epsilon():
            return "e"
        return " | ".join(
            "&".join(k for k, v in a.assignment if v)
