...
class Assignment(MutableMapping):
    ...
    def all_possible_assignments(propositions: tuple[str, ...]) -> list["Assignment"]:
        ...
        Guarantees a deterministic order.
        """
        if not propositions:
            return [Assignment({})]
        p = propositions[0]
    ...
    @staticmethod
    def more_than_one_true_proposition(
        propositions: set[str],
    ) -> set["FrozenAssignment"]:
        return {
            a.to_frozen()
    ...
    @staticmethod
    def where(*true_propositions: str, propositions: set[str]) -> "Assignment":
...
class FrozenAssignment:
    ...
    def to_label(self) -> str:
        cnf = [
            f'{"!" if not truth_value else ""}{p}'
            for p, truth_value in self.assignment
        ]
        return " & ".join(sorted(cnf, key=lambda x: x[1:] if x[0] == "!" else x))

    def to_string(self) -> list[str]:
        return [f"{p}" for p, truth_value in self.assignment if truth_value]

    def get_true_propositions(self) -> frozenset[str]:
        return frozenset(p for p, v in self.assignment if v)

    def __eq__(self, other):
        if not isinstance(other, FrozenAssignment):
            return False
