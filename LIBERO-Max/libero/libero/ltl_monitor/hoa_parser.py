"""HOA (Hanoi Omega-Automata) format parser into ``LDBA``.

Ported from Neuralsym-VLA/ltl_benchmark/automata/hoa_parser.py. Handles
epsilon transitions emitted by Rabinizer when invoked with the ``-e`` flag.
"""
...
class HOAParser:
    """A parser for LDBAs given in the HOA format.

    Handles epsilon transitions as output by Rabinizer with ``-e``.
    """
    ...
    def parse_propositions(self) -> None:
        ...
        if self.propositions is None:
            ...
            if not set(self.propositions_in_hoa).issubset(self.propositions):
                raise ValueError(
                    "Error parsing HOA. Found propositions in header that "
                    "do not match given propositions."
                )

    def find_and_parse_ap_line(self) -> list[str]:
        for num, line in enumerate(self.lines):
            if line.startswith("AP:"):
                return self.parse_ap_line(line.split(":", 1)[1].strip(), num)
    ...
    def parse_ap_line(value: str, line_number: int) -> list[str]:
        ...
        if int(num_props) != len(props):
            raise ValueError(
                f"Error parsing HOA at line {line_number}. "
                f"Expected {num_props} propositions."
            )
    ...
    def parse_header(self) -> None:
        ...
        if not found_start:
            raise ValueError(
                f"Error parsing HOA at line {self.line_number}. "
                "Missing required header field `Start`."
            )
    ...
    def parse_transition(self, source: int) -> None:
    ...
    def replace_numeric_propositions(self, label: str) -> str:
        ...
        return re.sub(
            regexp,
            lambda m: self.propositions_in_hoa[int(m.group(0))],
            label,
        )
    ...
    def peek(self, error_msg: Optional[str] = None) -> str:
        if self.line_number >= len(self.lines):
            raise ValueError(
                f"Error parsing HOA. Reached end of input."
                f'{"" if error_msg is None else f" {error_msg}"}'
            )
    ...
    def expect(self, expected, actual) -> None:
        if expected != actual:
            raise ValueError(
                f"Error parsing HOA at line {self.line_number}. Expected: {expected}."
            )
