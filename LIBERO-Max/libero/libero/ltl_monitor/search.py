"""Reach-avoid sequence extraction over an LDBA.

Ported from Neuralsym-VLA/ltl_benchmark/search/exhaustive_search.py. The
``ExhaustiveSearchSimple`` walks the LDBA from the supplied current states and
returns the shortest valid ``LDBASequence`` of ``(reach, avoid)`` pairs.
Neuralsym's neural value function is dropped — for benchmark monitoring we
just want the shortest reach-avoid path.
"""
...
from .automata import LDBA, LDBATransition
...
class Path:
    ...
    def prepend(
        self, reach: LDBATransition, avoid: set[LDBATransition]
    ) -> "Path":
...
class ExhaustiveSearchSimple:
    """Exhaustive shortest-sequence search across an LDBA (no neural net)."""
    ...
    def __call__(self, ldba: LDBA, ldba_states: List[int]) -> LDBASequence:
        ...
        processed_seqs: list[LDBASequence] = []
        for seq in seqs:
            reach_list, avoid = seq[0]
            suffix = seq[1:]

            avoid_assignments: list = []
            avoid_sets: list[frozenset[str]] = []
            for a, s in sorted(
                [(a, a.get_true_propositions()) for a in avoid],
            ...
            for reach in reach_list:
                ...
                if not any(avoid_set <= true_props for avoid_set in avoid_sets):
                    new_reach = frozenset(
                        [
                            Assignment.single_proposition(
                                p[0], self.propositions
                            ).to_frozen()
                            for p in reach
                            if p[1]
                        ]
    ...
    def all_sequences(
        self, ldba: LDBA, ldba_states: List[int], num_loops: int = 1
    ) -> List[LDBASequence]:
    ...
    def dfs(
        ...
        accepting_transition: Optional[LDBATransition],
        num_loops: int = 1,
    ) -> list[Path]:
        state_to_path_index[state] = len(current_path)
        neg_transitions: set[LDBATransition] = set()
        paths: list[Path] = []
        for transition in ldba.state_to_transitions[state]:
            ...
            if scc.bottom and not scc.accepting:
                ...
                if transition.target in state_to_path_index:  # cycle
                    if (
        ...
        for i in range(len(paths)):
            for j in range(i + 1, len(paths)):
                ...
                if len(paths[i]) < len(paths[j]):
                    if ExhaustiveSearchSimple.check_path_contained(
                        paths[j], paths[i]
                    ):
                        to_remove.add(j)
