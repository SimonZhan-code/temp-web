...
try:
    from libero.libero.ltl_monitor import get_task_ltl_spec
except ModuleNotFoundError:
    from libero.ltl_monitor import get_task_ltl_spec
...
class BDDLBaseDomain(SingleArmEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._ltl_goal_desc_map = None
        self._ltl_task_spec = None
    ...
    # step() is defined below with LTL label integration
    ...
    def get_ltl_propositions(self, regenerate: bool = False):
        """
        Get all atomic propositions for LTL construction.
        
        Automatically generates propositions from objects, predicates, and
        regions defined in the BDDL file. Generates:
        - Unary state propositions (obj.is_open, obj.is_closed, etc.)
        - Binary relation propositions (on(obj_i, obj_j), in(obj_i, obj_j))
        - Region propositions (in_region(obj, region))
        - Task goal propositions (from :goal in BDDL)
        Args:
            regenerate (bool): Force regeneration of propositions
        Returns:
            PropositionSet: All atomic propositions
        """
        if self.proposition_set is None or regenerate:
            self.proposition_generator = AtomicPropositionGenerator(
                self, verbose=False
            )
    ...
    def get_ltl_label(self) -> np.ndarray:
        """
        Get current truth assignment for all propositions.
        
        Returns:
            np.ndarray: Boolean array with shape (num_propositions,)
                        where label[i] is the truth value of propositions[i]
        """
        prop_set = self.get_ltl_propositions()
        return prop_set.get_label(self)
    
    def get_ltl_label_dict(self) -> dict:
        """
        Get current truth assignment as dictionary.
        
        Returns:
            dict: {prop_name: bool} mapping each proposition to its truth value
        """
        prop_set = self.get_ltl_propositions()
        return prop_set.get_label_dict(self)

    def get_ltl_goal_desc_map(self) -> dict:
        """
        Get a mapping from goal proposition names to human-readable descriptions.
        """
        if self._ltl_goal_desc_map is None:
            prop_set = self.get_ltl_propositions()
            self._ltl_goal_desc_map = {
                prop.name: prop.description
                for prop in prop_set.get_propositions_by_category("goal")
            }
        return self._ltl_goal_desc_map

    def get_ltl_task_spec(self) -> dict:
        """
        Get the registry or auto-generated task-level LTL specification.
        """
        if self._ltl_task_spec is None:
            task_id = os.path.splitext(os.path.basename(self.bddl_file_name))[0]
            self._ltl_task_spec = get_task_ltl_spec(
                self.get_ltl_propositions(),
                task_id=task_id,
                bddl_file_name=self.bddl_file_name,
            )
        return self._ltl_task_spec.as_dict()
    
    def step(self, action):
        """
        Step the environment and add LTL labels to info.

        Handles OSC_POSITION action conversion and adds proposition labels.
        """
        ...
        # Add LTL labels to info dict for use in training/logging
        info['ltl_label'] = self.get_ltl_label_dict()
        info['ltl_label_array'] = self.get_ltl_label()
        info['ltl_goal_desc'] = self.get_ltl_goal_desc_map()
        info['ltl_task_spec'] = self.get_ltl_task_spec()
