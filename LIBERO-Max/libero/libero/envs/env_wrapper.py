...
class ControlEnv:
    def __init__(
        ...
        **kwargs,
        ...
        # SafeLIBERO tasks share problem_names with regular LIBERO, so route them
        # (by bddl path) to the safety-aware per-arena domain; regular LIBERO is
        # unaffected.
        from libero.libero.envs.safety import get_safety_domain_class, is_safelibero_bddl

        domain_class = None
        if is_safelibero_bddl(bddl_file_name):
            domain_class = get_safety_domain_class(self.problem_name)
        if domain_class is None:
            domain_class = TASK_MAPPING[self.problem_name]
        self.env = domain_class(
            bddl_file_name,
            robots=robots,
