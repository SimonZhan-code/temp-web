"""LIBERO gym.Env wrapper with vectorized environment support."""
...
from libero.libero.benchmark.family import LiberoFamilySuite, get_libero_suite
try:
    from libero.libero.ltl_monitor import (
    ...
    from libero.ltl_monitor import (
        build_monitor_for_task,
        build_monitor_from_spec,
        infer_goal_props_from_info,
        infer_safety_props_from_label,
    )
from libero.env_wrapper.utils import (
    get_libero_image,
    get_libero_wrist_image,
    quat2axisangle,
)
from libero.env_wrapper.reconfigure_venv import ReconfigureSubprocEnv
from libero.env_wrapper.env_utils import (
...
class LiberoEnv(gym.Env):
    def __init__(self, cfg, num_envs, seed_offset, total_num_processes, worker_info):
        ...
        self.task_suite: LiberoFamilySuite = get_libero_suite(cfg.task_suite_name)
        ...
        self.enable_ltl_monitor = bool(cfg.get("enable_ltl_monitor", False))
    ...
    def get_env_fn_params(self, env_idx=None):
        ...
        for env_id in range(self.num_envs):
            ...
            env_fn_params.append(
                {
                    **base_env_args,
                    "bddl_file_name": self.task_suite.get_task_bddl_file_path(
    ...
    def _init_ltl_monitors(self):
        self.ltl_monitors = [None] * self.num_envs
        self._last_ltl_monitor_infos = [{} for _ in range(self.num_envs)]
        if not self.enable_ltl_monitor:
            return
        for env_id in range(self.num_envs):
            self._rebuild_ltl_monitor(env_id)

    def _rebuild_ltl_monitor(self, env_id, goal_props=None, safety_props=None):
        if not self.enable_ltl_monitor:
            return None
        task = self.task_suite.get_task(self.task_ids[env_id])
        monitor = build_monitor_for_task(
            task.name,
            goal_props=set(goal_props or ()),
            safety_props=set(safety_props or ()),
        )
        self.ltl_monitors[env_id] = monitor
        self._last_ltl_monitor_infos[env_id] = {}
        return monitor

    def _reset_ltl_monitors(self, env_idx=None):
        if not self.enable_ltl_monitor:
            return
        if env_idx is None:
            env_idx = np.arange(self.num_envs)
        for env_id in env_idx:
            monitor = self.ltl_monitors[env_id]
            if monitor is not None:
                monitor.reset()
            else:
                self._rebuild_ltl_monitor(env_id)

    def _update_ltl_monitor_infos(self, infos):
        if not self.enable_ltl_monitor:
            return infos

        labels = infos.get("ltl_label")
        goal_descs = infos.get("ltl_goal_desc")
        task_specs = infos.get("ltl_task_spec")
        monitor_infos = []

        for env_id in range(self.num_envs):
            label = (
                labels[env_id]
                if isinstance(labels, list) and env_id < len(labels)
                else None
            )
            goal_desc = (
                goal_descs[env_id]
                if isinstance(goal_descs, list) and env_id < len(goal_descs)
                else None
            )
            task_spec = (
                task_specs[env_id]
                if isinstance(task_specs, list) and env_id < len(task_specs)
                else None
            )

            monitor = self.ltl_monitors[env_id]
            if monitor is None and isinstance(task_spec, dict):
                monitor = build_monitor_from_spec(task_spec)
                self.ltl_monitors[env_id] = monitor
            if monitor is None:
                goal_props = infer_goal_props_from_info(goal_desc)
                safety_props = infer_safety_props_from_label(label)
                monitor = self._rebuild_ltl_monitor(
                    env_id,
                    goal_props=goal_props,
                    safety_props=safety_props,
                )

            if monitor is not None and isinstance(label, dict):
                monitor_info = monitor.step(label)
            else:
                monitor_info = {}
            monitor_infos.append(monitor_info)

        self._last_ltl_monitor_infos = monitor_infos
        infos["ltl_monitor"] = monitor_infos
        infos["ltl_accepted"] = [
            info.get("ltl_accepted", False) for info in monitor_infos
        ]
        infos["ltl_violated"] = [
            info.get("ltl_violated", False) for info in monitor_infos
        ]
        infos["ltl_reach_reward"] = [
            info.get("ltl_reach_reward", 0.0) for info in monitor_infos
        ]
        infos["ltl_safety_cost"] = [
            info.get("ltl_safety_cost", 0.0) for info in monitor_infos
        ]
        infos["ltl_reach_avoid_text"] = [
            info.get("ltl_reach_avoid_text", "") for info in monitor_infos
        ]
        return infos
    ...
    def _reconfigure(self, reset_state_ids, env_idx):
        ...
        self._reset_ltl_monitors(env_idx)
    ...
    def step(self, actions=None, auto_reset=True):
        ...
        infos = self._update_ltl_monitor_infos(infos)
